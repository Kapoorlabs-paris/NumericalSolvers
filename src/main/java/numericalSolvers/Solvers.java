...
public class Solvers {
	...
	/**
	 * Quartic solvers f0 is the coefficient of the zeroth power. f4 is the
	 * coefficient of the fourth power.
	 * 
	 * p = new double[] { f0, f1, f2, f3, f4 }
	 * 
	 * If you are solving an equation of lower order just put that coefficient to zero and re use the same function everywhere in your code
	 * 
	 * @V Kapoor
	 */
	...
	public static ArrayList<Pair<Integer, Double>> SolveDepressedQuartic(double c0, double c1, double c2) {
		...
		double delta = c1sqr * (-rat27 * c1sqr + rat4 * c2 * (rat36 * c0 - c2sqr))
		...
		if (delta > zero) {
			...
			if (c2 < zero && a1 < zero) {
				...
				// Four simple roots
				ArrayList<Pair<Integer, Double>> RootmapLocal = SolveCubic(
						new double[] { c1sqr - rat4 * c0 * c2, rat8 * c0, rat4 * c2, -rat8 });
	...
	public static ArrayList<Pair<Integer, Double>> SolveQuadratic(double[] p) {
		...
		double disc = q1 * q1 - 4 * q0;
		double rootA = (-q1 + disc) / 2.0;
		double rootB = (-q1 - disc) / 2.0;
		Rootmap.add(new ValuePair<Integer, Double>(1, rootA));
		Rootmap.add(new ValuePair<Integer, Double>(1, rootB));
